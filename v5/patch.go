...
func (p Patch) set(doc *container, op Operation, options *ApplyOptions) error {
	path, err := op.Path()
	if err != nil {
		return fmt.Errorf("set operation failed to decode path: %w", ErrMissing)
	}

	con, key := findObject(doc, path, options)

	if con == nil {
		return fmt.Errorf("set operation does not apply: doc is missing path: \"%s\": %w", path, ErrMissing)
	}

	val := op.value()

	if val.isNull() {
		return fmt.Errorf("set operation does not apply: path is null: %w", ErrInvalid)
	}

	err = con.set(key, val, options)
	if err != nil {
		return fmt.Errorf("error in set for path: '%s': %w", path, err)
	}
...
func (p Patch) incr(doc *container, op Operation, options *ApplyOptions) error {
	path, err := op.Path()
	if err != nil {
		return fmt.Errorf("incr operation failed to decode path: %w", err)
	}

	con, key := findObject(doc, path, options)

	if con == nil {
		return fmt.Errorf("incr operation does not apply: doc is missing path: \"%s\": %w", path, ErrMissing)
	}
	...
	if err != nil {
		...
		if err != nil {
			return fmt.Errorf("error in incr for path: '%s': %w", path, err)
		}
	}

	if val.isNull() {
		return fmt.Errorf("incr operation does not apply: path is null: %w", ErrInvalid)
	}
	...
	case json.Number:
		f, err := inc.Float64()
		if err != nil {
			return fmt.Errorf("incr value must be number: %w", err)
		}
		i += f
	default:
		return fmt.Errorf("incr value must be number: %w", ErrInvalid)
	}
